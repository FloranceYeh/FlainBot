...
class FrontendStaticTests(unittest.TestCase):
    ...
    def test_planner_page_references_assets_and_nodes(self):
        ...
        self.assertIn("graphNodeCenter", js)
        self.assertIn("nodeDragPosition", js)
        self.assertIn("nodePositionFromCenter", js)
        self.assertIn("function normalizeGraphCenter()", js)
        self.assertGreaterEqual(js.count("normalizeGraphCenter();"), 4)
        self.assertIn("nodePositionFromCenter(canvasPointFromEvent(event))", js)
        self.assertIn("nodeDragPosition(dragState.value, event, viewportState.scale)", js)
        self.assertNotIn("position ? Math.max(0, position.x)", js)
        self.assertNotIn("node.x = Math.max(0", js)
        self.assertNotIn("node.y = Math.max(0", js)
